...
The License section is for automating the application of Licensee to Instances while provisioning. Licenses can be added to |conduit| and then attached to images. |conduit| will then apply the license to Instances provisioned using the images with license attached. Licenses can be configured for single or multiple Tenants.
...
#. In the New License modal, enter the following:
   ...
   * License Type
      Windows
   * Name
      Name of the License in |conduit|
   * License Key
   ...
   * Description
      License description displayed in the Licenses list in |conduit| . Helpful for identifying License after creation
   * Virtual Images
...
When a Virtual Image is added to a license, |conduit| will automatically apply the License to Instances configured with the Virtual Image during provisioning, including Instance Types with a Node Type that is configured with the Virtual Image, or if the image is selected when using generic Cloud Instances types (VMware, AWS, Nutanix, Openstack etc). Virtual Images can be removed from a License by editing the License.
